...
def get_reenrollments_same_lineage(patient_WGS_data_fName, lineage_col='Coll2014'):
    ...
    # separate patients we can confirm had the same lineage from others. This will necessarily drop patients who had sequencing of only 1 or neither sample
...
def extract_lineages(df, name_col, out_dir=None):
    ...
    for sample in df[name_col].unique():
        ...
        if os.path.isfile(f"{out_dir}/{sample}/lineage/fast_lineage_caller_output.txt"):
            ...
            df_flc[name_col] = df_flc['Isolate'].str.split('_').str[0]
            
            for col in df_flc.columns:
                if col not in ['Isolate', name_col]:
