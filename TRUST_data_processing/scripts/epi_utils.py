...
log_metadata_vars = ['F2', 'TTP_baseline', 'peth_value_baseline']
...
cc_df = pd.read_csv("~/MtbLongitudinalDiversity/data/drug_CC.csv")
...
def get_all_available_MICs_single_drug(df_trust_patients, drug, id_vars=['Original_ID', 'pid', 'SampleID'], baseline_only=True):
    ...
    if baseline_only:
        ...
        df_MIC_single_drug = df_trust_patients[id_vars + [f's_{drug.lower()}mic_sputum_specimen_1', f's_{drug.lower()}mic_sputum_specimen_2']]
    
        # get the MIC testing media too
        # PZA doesn't have a testing media field because it was all MGIT
        if drug != 'PZA':
            df_MIC_methods_single_drug = df_trust_patients[id_vars + [f's_{drug.lower()}micmeth_sputum_specimen_1', f's_{drug.lower()}micmeth_sputum_specimen_2']]
        else:
            df_MIC_methods_single_drug = df_trust_patients[id_vars]

    # get all available MICs, taking the earliest one
    else:

        df_MIC_single_drug = df_trust_patients[id_vars + list(df_trust_patients.columns[df_trust_patients.columns.str.contains(f's_{drug.lower()}mic_sputum_specimen')])]
    
        # get the MIC testing media too
        # PZA doesn't have a testing media field because it was all MGIT
        if drug != 'PZA':
            df_MIC_methods_single_drug = df_trust_patients[id_vars + list(df_trust_patients.columns[df_trust_patients.columns.str.contains(f's_{drug.lower()}micmeth')])]
        else:
            df_MIC_methods_single_drug = df_trust_patients[id_vars]

    df_MIC_single_drug = df_MIC_single_drug.set_index(id_vars)
    df_MIC_methods_single_drug = df_MIC_methods_single_drug.set_index(id_vars)
...
def get_measured_MICs(df_RedCap, id_vars=['Original_ID', 'pid', 'SampleID'], baseline_only=True):
    ...
    for drug in drugs_lst:
        ...
        df_single_drug = get_all_available_MICs_single_drug(df_RedCap, drug, id_vars=id_vars, baseline_only=baseline_only)
...
def dummy_encode_lineages(df, lineage_col, binary=False):
    ...
    if binary:        
        ...
        # delete the majority lineage and return everything else
...
def fit_cox_hazard_ratio_model_with_single_penalty(df, df_outcome, cols_lst, event_col, time_col, MIC_type='none', include_drugs=drugs_lst, binarize_drugs=[], stratify_variables=[], non_linear_term_variables=[], interact_MIC_lineage=False, interact_indel_change_lineage=False, cluster_col=None, penalty_type='L2', penalize_features=[], binary_lineage=False):
    ...
    for _, alpha in enumerate(alphas_lst):
        ...
        for i, (train_index, test_index) in enumerate(skf.split(df_model_processed, df_model_processed[event_col])):
            ...
            # initialize model
            if penalty_type == 'L1':
                cph = lifelines.CoxPHFitter(penalizer=penalty_array, l1_ratio=1)
            else:
    ...
    print(f"Fitting model with {penalty_type} penalty of {best_penalty} with {param_to_maximize} = {np.max(cv_results.groupby('alpha')[param_to_maximize].mean())}")
...
def fit_cox_hazard_ratio_model_with_ElasticNet(df, df_outcome, cols_lst, event_col, time_col, MIC_type='none', include_drugs=drugs_lst, binarize_drugs=[], stratify_variables=[], non_linear_term_variables=[], interact_MIC_lineage=False, interact_indel_change_lineage=False, cluster_col=None, penalty_type=None, penalize_features=[], binary_lineage=False):
    ...
    # perform cross-validation to select the strength of the L2 penalty
    alphas_lst = np.logspace(-5, 5, 11)
    ratios_lst = np.linspace(0.1, 0.9, 9)
    
    param_to_maximize = 'log_likelihood'
    
    cv_results = pd.DataFrame(columns = ['alpha', 'CV', 'L1_ratio', param_to_maximize])
    k = 0
        
    for _, alpha in enumerate(alphas_lst):
        
        for _, ratio in enumerate(ratios_lst):
            ...
            for i, (train_index, test_index) in enumerate(skf.split(df_model_processed, df_model_processed[event_col])):
                ...
                # initialize model
                cph = lifelines.CoxPHFitter(penalizer=penalty_array, l1_ratio=ratio)
                ...
                # cph.log_likelihood_ returns the LL on the training data. cph.score(df) returns the LL on a new dataset (pass in the test data)
                cv_results.loc[k, :] = [alpha, i+1, ratio, cph.score(df_model_processed.iloc[test_index])]
                k += 1
            
    # Higher log-likelihood is a better model is better
    best_ratio, best_penalty = cv_results.groupby(['L1_ratio', 'alpha'])[param_to_maximize].mean().sort_values().index.values[-1]
    print(f"Fitting model with L1 ratio of {best_ratio} and penalty = {best_penalty} with {param_to_maximize} = {np.max(cv_results.groupby(['L1_ratio', 'alpha'])[param_to_maximize].mean())}")
    ...
    # fit the new model
    cph = lifelines.CoxPHFitter(penalizer=penalty_array, l1_ratio=best_ratio)
...
def fit_cox_models_all_imputations(df_trust_patients, df_pred_combined, TRUST_phenos, df_outcome, cols_lst, event_col, time_col, alpha=0.05, invert_OR=True, exclude_resistance=False, MIC_type=None, include_drugs=[], binarize_drugs=[], tb_deaths_only=False, stratify_variables=[], non_linear_term_variables=[], interact_MIC_lineage=False, interact_indel_change_lineage=False, cluster_col=None, penalty_type=None, penalize_features=[], binary_lineage=False):
    ...
    for imp_num in imp_lst:
        ...
        if penalty_type in ['L1', 'L2']:
            model_func = fit_cox_hazard_ratio_model_with_single_penalty
        elif penalty_type == 'ElasticNet':
            model_func = fit_cox_hazard_ratio_model_with_ElasticNet
        else:
