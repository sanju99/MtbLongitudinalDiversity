...
parser.add_argument("-i", dest='patient_data_fName', type=str, required=True, help='Full path to a filename for the RedCap data and WGS data already merged. Created by combine_patient_WGS_data.py')
parser.add_argument("-d1", dest='raw_data_dir', type=str, default='~/TRUST_data_processing/raw_data', help='Directory with data sent from collaborators with no processing done by me.')
parser.add_argument("-d2", dest='processed_data_dir', type=str, default='~/TRUST_data_processing/processed_data', help='Directory of data that has had some additional processing done by me.')

cmd_line_args = parser.parse_args()

patient_data_fName = cmd_line_args.patient_data_fName
raw_data_dir = cmd_line_args.raw_data_dir
processed_data_dir = cmd_line_args.processed_data_dir
...
df_trust_patients['diabetes'] = df_trust_patients['bl_medhx___5']

# 2 = female, 1 = male.
df_trust_patients['screen_sex'] = df_trust_patients['screen_sex'].replace(2, 0)
...
############################## STEP 2. MERGE IN PK DRUG AUC, CHEST X-RAY PREDICTIONS, AND ADHERENCE DATA ##############################
    
    
df_drug_AUC = pd.read_csv(f"{processed_data_dir}/PK_AUC_predictions.csv")
...
pli_predictions = pd.read_csv(f"{processed_data_dir}/CXR_predictions_baseline.csv")

df_trust_patients = df_trust_patients.merge(pli_predictions, on='pid', how='left')

adherence_data = pd.read_csv(f"{raw_data_dir}/overall_adherence.csv")

df_trust_patients = df_trust_patients.merge(adherence_data[['pid', 'adherence_rate_by_observed']], on='pid', how='left')


##################### STEP 4: MERGE IN TTP AND SMEAR INFORMATION, WHICH ARE IN DIFFERENT DATAFRAMES CREATED BY prep_data_for_culture_imputation.py #####################


df_TTP_smear = pd.read_csv(f"{processed_data_dir}/TTP_smear_results.csv")

# only keep TTP and smear from the first two weeks. The smear grade has also already been re-encoded to be properly ordered
df_TTP = df_TTP_smear.query("culture_sample_num <= 2").sort_values(['pid', 'culture_sample_num']).drop_duplicates(subset='pid')
df_smear = df_TTP_smear.query("smear_sample_num <= 2").sort_values(['pid', 'smear_sample_num']).drop_duplicates(subset='pid')

df_trust_patients = df_trust_patients.merge(df_TTP[['pid', 'TTP']].rename(columns={'TTP': 'TTP_baseline'}), on='pid', how='left')
df_trust_patients = df_trust_patients.merge(df_smear[['pid', 'smear_grade']].rename(columns={'TTP': 'smear_grade_baseline'}), on='pid', how='left')
...
include_pids = df_trust_patients.sort_values(['unique_patient', 'pid']).drop_duplicates(subset='unique_patient', keep='first').pid.values

# can't do drop_duplicates directly because it will keep only the first WGS sample for each pid
df_trust_patients = df_trust_patients.query("pid in @include_pids")
...
print(f"Keeping {df_trust_patients.pid.nunique()} pids with {df_trust_patients.dropna(subset='F2').SampleID.nunique()} WGS samples for downstream analyses\n")
df_trust_patients.to_csv(os.path.join(processed_data_dir, f"{current_date}_cleaned_patient_outcomes_data.csv"), index=False)
...
df_final.drop_duplicates().to_csv(os.path.join(processed_data_dir, "tx_outcomes.csv"), index=False)