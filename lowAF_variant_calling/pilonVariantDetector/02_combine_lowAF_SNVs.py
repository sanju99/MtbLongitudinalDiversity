import numpy as np
import pandas as pd
import glob, os, argparse, time, vcf, warnings, sys
...
parser.add_argument("-csv", dest="lowAF_variants_H37Rv_CSV_fName", type=str, help='CSV file of low frequency variants called using the H37Rv reference genome, after excluding low confidence sites.')
...
lowAF_variants_H37Rv_CSV_fName = cmd_line_args.lowAF_variants_H37Rv_CSV_fName
...
keep_cols = ['REF', 'ALT', 'QUAL', 'FILTER', 'DP', 'TD', 'BQ', 'MQ', 'BC', 'IC', 'DC', 'XC', 'AF', 'SAF', 'SAR']
...
df_ground_truth.to_csv(f"{save_dir}/pilon/ground_truth.csv", index=False)
...
df_lowAF_variants_H37Rv_asm = pd.read_csv(lowAF_variants_H37Rv_CSV_fName)
...
# these are the same information, so need to switch things around because when using the apply_pilon_lowAF_QCfilters function, it will be screened out due to the AF max of 0.75
# already recomputed AF in 01_write_lowAF_BED.py
...
# then after the switch above, apply QC filters to keep only the high quality ones and remove those with fixed allele frequencies. The other filters don't need to be applied here
# df_lowAF_variants_H37Rv_asm = apply_freebayes_lowAF_QCfilters(df_lowAF_variants_H37Rv_asm)
df_lowAF_variants_H37Rv_asm = apply_pilon_lowAF_QCfilters(df_lowAF_variants_H37Rv_asm)

df_lowAF_variants_H37Rv_asm[keep_cols].to_csv(f"{save_dir}/pilon/H37Rv_detected.csv", index=False)



def get_confusion_matrix_lowAF_detection(fName_personal_asm_variants, fName_H37Rv_variants):
    
    keep_cols = ['POS', 'REF', 'ALT', 'AF']
    
    df_personal_asm_variants = pd.read_csv(fName_personal_asm_variants)[keep_cols]
    df_H37Rv_variants = pd.read_csv(fName_H37Rv_variants)[keep_cols]
    ...
    # df_combined = df_personal_asm_variants.merge(df_H37Rv_variants, left_on=keep_cols, right_on=[f"H37Rv_{col}" for col in keep_cols], how='outer')
...
get_confusion_matrix_lowAF_detection(f"{save_dir}/pilon/ground_truth.csv", 
                                     f"{save_dir}/pilon/H37Rv_detected.csv"
                                    )