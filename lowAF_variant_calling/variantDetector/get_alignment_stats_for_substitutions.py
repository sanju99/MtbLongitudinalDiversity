...
out_dir = f"{os.path.dirname(os.path.dirname(bam_fName))}/freebayes"
...
# # compute rolling average of coverage. Smaller than 100 is too small, not enough smoothing.
# window_size = 5000
# df_depth['LONG_COV_LEFT_ROLLING_AVG'] = df_depth['COV'].rolling(window=window_size, min_periods=1, closed='right').mean()

# # for this one, compute the rolling average as you would from the left, but reverse the values beforehand. This is the easiest way. Then reverse them again
# df_depth['LONG_COV_RIGHT_ROLLING_AVG'] = df_depth['COV'][::-1].rolling(window=window_size, min_periods=1, closed='right').mean()[::-1]

# # take the maximum at each site
# df_depth['LONG_COV_MAX_ROLLING_AVG'] = np.max([df_depth['LONG_COV_LEFT_ROLLING_AVG'], df_depth['LONG_COV_RIGHT_ROLLING_AVG']], axis=0)
...
df_variants.query("AF >= 0.05 & AF <= 0.95").to_csv(f"{out_dir}/lowAF_SNPs.csv", index=False)
df_variants.query("AF > 0.95").to_csv(f"{out_dir}/fixed_SNPs.csv", index=False)