...
def write_lowAF_variant_table(model_dir, fixed_thresh=0.95):
    ...
    assert len(df_candidate_SNPs.loc[df_candidate_SNPs['SAF'] + df_candidate_SNPs['SAR'] != df_candidate_SNPs['AO']]) == 0

    # but extreme values are bad, so get the distance from 0.5. Ideally want it to be close to 0.5
    df_candidate_SNPs['SAF_prop_deviation_from_half'] = np.abs(0.5 - (df_candidate_SNPs['SAF'] / df_candidate_SNPs['AO']))
    ...
    pos_only_false_variants = pd.DataFrame(df_candidate_SNPs_ground_truth.query("F2 <= @F2_max").groupby('POS').Real.mean()).query("Real==0").index.values
    ...
    # pos_only_false_variants = list(set(pos_only_false_variants).intersection(pos_multiple_samples))
    ...
    # ------------------------------------------------------------------------------------------------------------------------
    predictors = ["COV_RATIO", "CLIPPED_BASES_RATIO", "DISCORDANT_READS_RATIO", "Mean_BQ_ALT_allele", "SAF_prop_deviation_from_half"]
