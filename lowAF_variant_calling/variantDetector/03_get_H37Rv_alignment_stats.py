...
if len(df_variants) == 0:
    ...
    # save empty dataframes so that snakemake registers that the rule finished. Then exit the script
    df_variants.to_csv(f"{out_dir}/lowAF_SNVs.csv", index=False)
    df_variants.to_csv(f"{out_dir}/fixed_SNVs.csv", index=False)
...
df_variants.query("AF >= 0.05 & AF <= 0.95").to_csv(f"{out_dir}/lowAF_SNVs.csv", index=False)
