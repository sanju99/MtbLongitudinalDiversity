...
parser.add_argument("--F2_min", type=float, default=0, help='Minimum F2 (exclusive) to keep samples for')
...
num_ground_truth_samples = len(df_trust_patients.query("SampleID in @ground_truth_samples & F2 > @F2_min & F2 <= @F2_max"))

non_ground_truth_samples = set(os.listdir(H37Rv_ref_dir)) - set(ground_truth_samples)

num_non_ground_truth_samples = len(df_trust_patients.query("F2 > @F2_min & F2 <= @F2_max & SampleID in @non_ground_truth_samples"))

print(f"{num_ground_truth_samples} ground truth samples, {num_non_ground_truth_samples} validation samples")


def write_lowAF_variant_table(model_dir, fixed_thresh=0.95, base_qual_min=20):
    ...
    # already ran split_MNPs_into_SNPs in 03_get_H37Rv_alignment_stats.py
    SNP_data_files = glob.glob(f"{H37Rv_ref_dir}/*/freebayes/lowAF_SNPs.csv")
    ...
    # already ran split_MNPs_into_SNPs before creating ground_truth.csv
    ground_truth_SNP_data_files = glob.glob(f"{personal_ref_dir}/*/lowAF_comparison/ground_truth.csv")
    ...
    df_ground_truth = pd.concat(df_ground_truth).merge(df_trust_patients[['pid', 'SampleID', 'Coll2014', 'Freschi2020', 'Lineage', 'F2']], on='SampleID', how='left').query("F2 > @F2_min & F2 <= @F2_max")
    assert len(df_ground_truth.query("REF.str.len() != 1")) == 0
    assert len(df_ground_truth.query("ALT.str.len() != 1")) == 0
    ...
    # exclude positions with a mean base quality < 20. That's just too low to be real
    df_candidate_SNPs = df_candidate_SNPs.query("Mean_BQ_ALT_allele >= @base_qual_min")
    ...
    # add pid
    df_candidate_SNPs = df_candidate_SNPs.merge(df_trust_patients[['pid', 'SampleID', 'Coll2014', 'Freschi2020', 'Lineage', 'F2']], on='SampleID', how='left').query("F2 > @F2_min & F2 <= @F2_max")
    print('GENE' in df_candidate_SNPs.columns)
    ...
    # get a list of positions that are never Real and that also occur in more than 1 Lineage. These are suspicious 
    ...
    pos_only_false_variants = pd.DataFrame(df_candidate_SNPs_ground_truth.query("F2 > @F2_min & F2 <= @F2_max").groupby('POS').Real.mean()).query("Real==0").index.values

    # require that they had a mean Real of 0, but that it wasn't based on a single lineage. Exclude mixed lineages for this
    # have to first remove the NAs, which are the variants missed by the H37Rv detection. Most are nesar the fixed boundary or some issue with mixed lineages
    # pos_multiple_samples = pd.DataFrame(df_candidate_SNPs_ground_truth.loc[~pd.isnull(df_candidate_SNPs_ground_truth['Lineage'])].query("~Lineage.str.contains(',')").groupby('POS')['Lineage'].nunique()).query("Lineage > 1").index.values
    
    # require that they were also False in at least 2 pids
    pos_multiple_pids = pd.DataFrame(df_candidate_SNPs_ground_truth.query("F2 > @F2_min & F2 <= @F2_max & Real == 0").groupby('POS').pid.nunique()).query("pid > 1").index.values
    ...
    df_val = df_val.query("AF <= @fixed_thresh").query("F2 > @F2_min & F2 <= @F2_max").drop_duplicates()
...
def write_fixed_variable_table(model_dir, fixed_thresh=0.95):
    ...
    fixed_SNP_data_files = glob.glob(f"{H37Rv_ref_dir}/*/freebayes/fixed_SNPs.csv")
    ...
    df_fixed_SNPs = df_fixed_SNPs.merge(df_trust_patients[['pid', 'SampleID', 'Coll2014', 'Freschi2020', 'Lineage', 'F2']], on='SampleID', how='left').query("F2 > @F2_min & F2 <= @F2_max")
...
print(f"Keeping only WGS samples with F2 in ({F2_min}, {F2_max}]")
