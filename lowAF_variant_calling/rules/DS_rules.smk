...
rule freebayes_split_haplotypes_separate_records:
    input:
        vcf_file_fixed = f"{sample_out_dir}/freebayes/{{sample_ID}}.cleaned.vcf",
    output:
        temp_file = temp(f"{sample_out_dir}/freebayes/{{sample_ID}}.cleaned.excludeLowConf.temp.vcf"),
    ...
    params:
        ...
        high_homology_regions = "/home/sak0914/MtbLongitudinalDiversity/lowAF_variant_calling/references/BED_files/10_high_homology_regions_Chromosome.bed",
        ref_genome = os.path.join(primary_directory, references_dir, "ref_genome", "H37Rv_NC_000962.3.fna"),
    conda:
        f"{conda_directory}/envs/variant_normalization.yaml"
    shell:
        ...
        bcftools norm --multiallelics -both {input.vcf_file_fixed} --fasta-ref {params.ref_genome} | bedtools subtract -a '-' -b {params.exclude_regions} -header > {output.temp_file}
        
        bedtools subtract -a {output.temp_file} -b {params.high_homology_regions} -header | bcftools sort > {output.vcf_file_split}
        """
...
rule exclude_low_conf_regions_pilon:
    ...
    shell:
        """
        bedtools subtract -a {input.vcf_SNPs} -b {params.exclude_regions} -header | bedtools subtract -a '-' -b {params.high_homology_regions} -header > {output.vcf_SNPs}
        bedtools subtract -a {input.vcf_indels} -b {params.exclude_regions} -header | bedtools subtract -a '-' -b {params.high_homology_regions} -header > {output.vcf_indels}
        """
...
rule get_TbtypeR_sites_pilon_VCF:
    input:
        vcf_file_gzip = f"{sample_out_dir}/pilon/{{sample_ID}}_full.vcf.gz",
    output:
        vcf_file_tbtyper_sites = f"{sample_out_dir}/pilon/{{sample_ID}}_TBtypeR_SNPs.vcf",
    params:
        tbtyper_sites = "/home/sak0914/MtbLongitudinalDiversity/direct_sputum/tbtyper_targets_Chromosome.bed",
    conda:
        f"{primary_directory}/envs/variant_calling.yaml"
    shell:
        """
        bedtools intersect -a {input.vcf_file_gzip} -b {params.tbtyper_sites} -header | bcftools norm --remove-duplicates > {output.vcf_file_tbtyper_sites}
        """