...
parser.add_argument('--num_max_mixed_lineages', type=int, dest='num_max_mixed_lineages', default=3, help='Most number of mixed lineages to consider. All FN scores from F2 - FN will be computed. 2 = only calculate the F2 score.')
...
num_max_mixed_lineages = cmd_line_args.num_max_mixed_lineages

output_file = os.path.join(os.path.dirname(allele_fractions_by_lineage), f"mixing_scores_Coll2014.csv")
...
def get_top_N_distinct_lineages(df_allele_fractions, num_mixed_lineages):
    ...
    df_allele_fractions = df_allele_fractions
    ...
    top_N_lineage_mafs = [df_allele_fractions['maf'].values[0]]
    ...
    for i, row in df_allele_fractions.iterrows():
        ...
        if len(top_N_lineages) < num_mixed_lineages:
            ...
            # check no overlap OR the mafs are more than 0.05 away from each other
            # for some deep sublineages (like 2.2.1.1 vs. 2.2.1, there may be more SNPs supporting one sublineage over another, and the mafs will be very different
            # in those cases, consider them separate values, even though the intersection between query and target will not be 0 because they are hierarchical
            # do np.min(np.abs(...)) so that if you have L2 = L2.2 = L2.2.1 all around the same value, but L2.2.1.1 at a much different value, it will only add the maf for L2/L2.2/L2.2.1 once
            if len(set(query_lineages).intersection(target_lineages)) == 0 or np.min(np.abs(np.array(top_N_lineage_mafs) - row['maf'])) > 0.05:
                top_N_lineages.append(row['lineage'])
                top_N_lineage_mafs.append(row['maf'])
...
# get the top distinct lineages with the highest MAFs. Distinct so that you don't consider like 2.2 and 2.2.1. Presumably they would be the same
df_FN = pd.DataFrame(columns = ['NumLineages', 'Score'])

for i, num_mixed_lineages in enumerate(np.arange(2, num_max_mixed_lineages + 1)):
    
    top_N_lineages = get_top_N_distinct_lineages(df_allele_fractions, num_mixed_lineages)
    print(f"Top {num_mixed_lineages}: lineages: {top_N_lineages}")
    ...
    df_FN.loc[i, :] = [num_mixed_lineages, M / D]
