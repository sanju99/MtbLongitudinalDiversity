...
parser.add_argument('--DP', type=float, dest='DP_THRESH', default=10, help='Total read depth threshold (inclusive) to consider SNPs high quality')
...
def check_is_snp(record):
    ...
    # requiring IC and DC to be 0 excludes too many things because there's some noise
    ...
    if 'IMPRECISE' in record.INFO.keys() or 'SVTYPE' in record.INFO.keys():
        ...
        if len(record.REF) == len(alt_allele) and record.REF != alt_allele and alt_allele != 'None' and alt_allele != '.':
            return True
...
def allele_category(record, qualThresh=10, presentThresh=0.75, DP_thresh=5, MQ_thresh=40, BQ_thresh=20):
    ...
    # the filter field is an empty list if it is PASS, else the list is non-empty
...
with open(OUT_FILE, "w+") as file:
    ...
    for i, fName in enumerate(file_paths):
        ...
        isolate_name = os.path.basename(fName).replace(".eff", "").replace(".vcf", "").replace("_variants", "").replace("_SNPs", "")
