...
if not os.path.isfile("~/MtbLongitudinalDiversity/data/nucleotide_runs_all_lengths.csv"):
    HT_regions = find_HT_regions(str(h37Rv_seq.seq))
    HT_regions['Region'] = HT_regions['POS'].map(h37Rv_coords_dict)
    HT_regions.to_csv("~/MtbLongitudinalDiversity/data/nucleotide_runs_all_lengths.csv", index=False)
...
def get_fixed_variants(fName, df_pid_metadata, AF_thresh=0.75, num_support_each_direction=5, MQ_thresh=40):
    ...
    df_variants = df_variants.query("AF > @AF_thresh & MQM >= @MQ_thresh").merge(df_pid_metadata[['pid', 'SampleID', 'Sampling_Week', 'F2', 'Lineage', 'Coll2014', 'Freschi2020']], on='SampleID')
    
    # only use the support in either direction filter for non-deletions. Because long deletions may not have enough reads on either side because those reads don't exist
    df_variants = pd.concat([df_variants.query("(REF.str.len() - ALT.str.len() > 10)"),
                             df_variants.query("~(REF.str.len() - ALT.str.len() > 10) & SAF >= @num_support_each_direction & SAR >= @num_support_each_direction")]
                           )
    
    # sort for readability
    df_variants = df_variants.sort_values(["pid", 'Sampling_Week']).set_index(["pid", 'SampleID', 'Sampling_Week']).reset_index()
...
def get_unfixed_variants(fName, df_pid_metadata, AF_thresh=0.05, AF_max=0.75, num_support_each_direction=2, MQ_thresh=40):
    ...
    sample_dir = os.path.dirname(fName)
    ...
    # subset using the AF thresholds and add metadata. SAP_prob > 0.05
    # keep only fixed variants and add metadata. SAP_prob > 0.05
    df_variants = df_variants.query("AF > @AF_thresh & AF <= @AF_max & MQM >= @MQ_thresh").merge(df_pid_metadata[['pid', 'SampleID', 'Sampling_Week', 'F2', 'Lineage', 'Coll2014', 'Freschi2020']], on='SampleID')
    ...
    # sort for readability
    df_variants = df_variants.sort_values(['Sampling_Week', 'POS']).set_index(["pid", 'SampleID', 'Sampling_Week']).reset_index()
    ...
    ###### ADD ANNOTATIONS FOR POSSIBLE FALSE LOW AF VARIANTS USING THE COVERAGE AND SNP DENSITY ROLLING AVERAGES ######
    ...
    if len(df_variants) > 0:
        ...
        if not os.path.isfile(f"{sample_dir}/coverage_plateau_sites.npy") or not os.path.isfile(f"{sample_dir}/high_density_SNP_sites.npy"):
            raise ValueError(f"Low quality site annotations have not yet finished for {sample}")
        
        coverage_plateau_sites = np.load(f"{sample_dir}/coverage_plateau_sites.npy")
        high_density_SNP_sites = np.load(f"{sample_dir}/high_density_SNP_sites.npy")
...
def get_matrix_of_variants_ROI(df_variants, pids_for_analysis, sample_2=True, binarize_fixed_variants=False, fixed_thresh=0.95, absent_thresh=0.05):

    if 'Paired_Sample_Num' in df_variants.columns:
        del df_variants['Paired_Sample_Num']
        
    df_variants = df_variants.merge(pids_for_analysis[['SampleID', 'Sampling_Week', 'Paired_Sample_Num']], how='inner', on=['SampleID', 'Sampling_Week'])
    ...
    if sample_2:
        ...
        return sample_1_variants.drop(variants_fixed_high_proportion, axis=1).astype(float)
    
    
    
def get_matrix_of_phase_variant_indels(data_dir, pids_for_analysis, HT_nucs, sample_2=True, binarize_fixed_variants=False, fixed_thresh=0.95, absent_thresh=0.05):

    # previously annotated phase variants in homopolymeric tracts
    df_variants = pd.concat([pd.read_csv(f"{data_dir}/fixed_variants.csv"), pd.read_csv(f"{data_dir}/unfixed_variants.csv")]).query("Phase_Variant==1 & Low_Qual != 1")
