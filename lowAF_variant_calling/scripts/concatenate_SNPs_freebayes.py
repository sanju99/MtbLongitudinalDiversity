...
parser.add_argument('--AF-thresh', type=float, dest='AF_THRESH', default=0.90, help='Alternative allele frequency threshold (exclusive) to consider variants present')
...
def allele_category(record, qualThresh=10, DPThresh=10, presentThresh=0.75):
    '''
...
def introduce_snps_single_seq(fName, h37Rv_region, START, END, qualThresh=10, DPThresh=10, presentThresh=0.75):
    ...
    for record in records:
        ...
        if check_is_snp(record):
            ...
            single_allele_type = allele_category(record, qualThresh, DPThresh, presentThresh)
...
with open(OUT_FILE, "w+") as file:
    ...
    for i, fName in enumerate(file_paths):
        ...
        seq_with_snps = introduce_snps_single_seq(fName, h37Rv_region, START, END, qualThresh=10, DPThresh=DP_THRESH, presentThresh=AF_THRESH)
